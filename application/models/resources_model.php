...
class Resources_model extends CI_Model {
	...
	*/
	...
	{
        /**
         * Add content for this function, in order to show some content in the
         * index page.
         */
		/* show all posts which del_status=0 or del_status=1 */
		if($show=='all')
			$this->db->select('id, UNIX_TIMESTAMP(post_date) as post_date, title, content, approval_status, del_status')->from('news')->order_by("post_date desc");
		/* show all post according to $del_status */
			...
			if($limit == NULL)
			    $this->db->select('id, UNIX_TIMESTAMP(post_date) as post_date, title, content, approval_status, del_status')->from('news')->where(array("approval_status"=> 1,"del_status"=>$del_status))->order_by("post_date desc");
			else
				$this->db->select('id, UNIX_TIMESTAMP(post_date) as post_date, title, content, approval_status, del_status')->from('news')->where(array("approval_status"=> 1,"del_status"=>$del_status))->order_by("post_date desc")->limit($limit, $offset);
		}

...
(function($) {
			...
			$.ajax({
				...
				success: function(result) {
					...
					var mapped = $.map(result, function(item) {
													return new ModelLibrary.DeviceModel(item); 
											    });
					var i = mapped.length;
	...
	ModelLibrary.DeviceModel = function(data) {
		...
		this.name = data.name;		
		this.id = data.id;
		// id here corresponds to {href: id }'s id and name corresponds to text:name's name
			
		this.expanded = ko.observable(false);

		var mapped = $.map(data.library, function(item) {

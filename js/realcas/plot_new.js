...
(function($) {
	...
	plotGraphs = function(container, output)
	{
		...
		container.html("")
		.append('<b class="graphbold">Graph:</b><select id="graph"></select>')
		.append('<div id="clickgroup"><a href="Javascript:void:(0)" id="log_plot" title="Log Y-axis" style="margin:0 10px ; display: inline-block"><i class="icon-check-empty"></i>Log Y-axis</a> <a href="Javascript:void:(0)" id="log_plot_X" title="Log X-axis" style="margin:0 10px; display:inline-block"><i class="icon-check-empty"></i>Log X-axis</a></div>') ;
		// .append('') ;
		...
		container.find('.graph-container').wrapAll('<div class="clearfix"></div>');
		    ...
		    if (($("#log_plot i").attr("class") == "icon-check") && ($("#log_plot_X i").attr("class") == "icon-check")) {

                    // jqPlotObject[position].log = false;
					$("#log_plot i").remove();
					$(this).prepend($("<i class='icon-check-empty' />"));
					jqPlotObject[position].plot.replot({data:[]});
					jqPlotObject[position].plot.replot({axes:{xaxis:{renderer: $.jqplot.LogAxisRenderer},x2axis:{renderer: $.jqplot.LogAxisRenderer}}});
			...
			else if (($("#log_plot i").attr("class") != "icon-check") && ($("#log_plot_X i").attr("class") == "icon-check")) {
					...
					$("#log_plot i").remove();
					$(this).prepend($("<i class='icon-check-empty' />"));
					// $("#log_plot_X i").attr("class","icon-check-empty");
					jqPlotObject[position].plot.replot({data:[]});
					jqPlotObject[position].plot.replot({axes:{xaxis:{renderer: $.jqplot.LinearAxisRenderer},x2axis:{renderer: $.jqplot.LinearAxisRenderer}}});
					jqPlotObject[position].plot.replot({axes:{yaxis:{renderer: $.jqplot.LinearAxisRenderer},y2axis:{renderer: $.jqplot.LinearAxisRenderer}}});
					jqPlotObject[position].plot.replot({ resetAxes: true });

...
CodeMirrorUI.prototype = {
  ...
  find: function( start ) {
    ...
    */
    ...
    if (found) {
      ...
      var self = this;
      confirm("No more matches.  Should we start from the top?", function(isconfirm) {
        if(isconfirm) {
          self.cursor = self.mirror.getSearchCursor(findString, 0, !isCaseSensitive);
          found = self.cursor.findNext();
          if (found) {
            self.mirror.setSelection(self.cursor.from(),self.cursor.to())
            //this.cursor.select();
          } else {
  ...
  replace: function() {
    ...
    //if (this.replaceAll.checked) {
      ...
      var cursor = this.mirror.getSearchCursor(findString, 0, !isCaseSensitive);
  ...
  jump: function() {
    var self = this;
    prompt("Jump to line:", function(line) {;
      if (line && !isNaN(Number(line))) {
        self.mirror.setCursor(Number(line) - 1,0);
        //self.mirror.setSelection({line:Number(line),ch:0},{line:Number(line)+1,ch:0});
        self.mirror.focus();
      }
    });
  },

...
function changeParam(event){
	$current = $("#"+event.target.parentNode.id);
	if($current.attr("type")=="NMOS")
	{
		var tmp_dialog = "<div id='param_dialog' style='display:none;'>"
					+"<select id='sel'><option value=1> test select option </option><option value=2> test select option2 </option><option value=3> test select option3 </option></select>"
...
function drawLine(event){
	...
	$("#workspace").bind({
		click:function(event){
			 ...
			 */
			if(clicks){
				$(tmp_l).attr("d",tmp_d + "L" + putGrid(event.pageX) + " " + putGrid(event.pageY));
			...
			tmp_l = makeSVG("path",{
				...
				"stroke"		:"#000000",
				"stroke-width"	:"1",
				"d"				:"M " + putGrid(event.pageX) + " " + putGrid(event.pageY)
			});
		...
		mousemove:function(event){
			...
			$("#click_guide").attr({
				"cx"	:""+putGrid(event.pageX),
				"cy"	:""+(putGrid(event.pageY))
			});
		},
		dblclick:function(event){

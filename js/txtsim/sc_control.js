...
function startMove(event){
	...
	var transform = $current.find(".drawing").attr("transform");
	var parts = transform.match(/(-?\d+)/g);
	a=parseInt(parts[0]); b=parseInt(parts[1]); c=parseInt(parts[2]);
	d=parseInt(parts[3]); e=parseInt(parts[4]); f=parseInt(parts[5]);
	...
	switch($current.attr("rota-stat")){
		case "1": break;
		case "2": f-=40;break;
		case "3": e-=40;f-=40;break;
		case "4": e-=40;break;
		case "5": f-=40;break;
		case "6": e-=40;f-=40;break;
		case "7": e-=40;break;
		case "8": break;
	}
}
...
function moveIt(event){
	...
	var y2 = y1 + y0;
	switch($current.attr("rota-stat")){
		case "1": e=x2;f=y2;break;
		case "2": e=x2;f=y2-40; break;
		case "3": e=x2-40;f=y2-40; break;
		case "4": e=x2-40;f=y2; break;
		case "5": e=x2;f=y2-40;break;
		case "6": e=x2-40;f=y2-40; break;
		case "7": e=x2-40;f=y2; break;
		case "8": e=x2;f=y2; break;
	}
...
function endMove(event){
	...
	$current.find(".drawing").children("rect").attr({ "width": tempwidth });
	$current.find(".drawing").children("rect").attr({ "height": tempheight });
	...
	if($current.attr("class") == "workspace_component"){
		...
		// 	case "4": f+=40; break;
		// }
		var x2=putGrid(parts[4]), y2=putGrid(parts[5]);
		$current.find(".drawing").attr({
			"transform":"matrix("+a+","+b+","+c+","+d+"," + x2 + "," + y2 + ")"
		});
		...
		if($current.attr("type")=="node"){
			...
			switch($current.attr("rota-stat")){
				case "2": case "4": case "6": case "8":
				if($current.attr("term")=="2"){
					$current.find(".text").attr({"transform":"translate(" + putGrid(e-20) + "," + putGrid(f-20) + ")"});
				}
				else{
					$current.find(".text").attr({"transform":"translate(" + putGrid(e) + "," + putGrid(f) + ")"});
				}
...
function changeParam(event){
	...
	{	
		...
		$.ajax({
			...
			success: function(result) { 
				...
				var tmp_dialog = "<div class='model-library' title='" + $current.attr("type").substring(5) + "' id='param_dialog' style='display:none;'>" 
							+"<select id='sel'><option value='' disabled selected>Choose ModelCard from your Library</option>";
				...
				$.each(output_result, function(i, item){	
					tmp_dialog = tmp_dialog +"<label><span>"+ item["name"] + "[" + item["unit"] +"] </span>" 
								+ "<input class='param_value' maxlength='8' type='text' value=" + item["default"]
								+ " id='"+item["name"]+"'></input></label>";									
					// tmp_dialog += "<br>" ; 
				});
	...
	else
	{
		if($current.attr("type")=="dcv" || $current.attr("type")=="dcc"){
			var tmp_dialog = "<div id='param_dialog' style='display:none;' type='DC'>"
			...
			var tmp_dialog = "<div id='param_dialog' style='display:none;' type='pulse'>"
						+ "<label><span>Name</span><input id='param_name' maxlength='8'/></label>"
						+ "<label><span>V<sub>initial</sub></span><input id='v_initial' class='param_value' /></label>"
						+ "<label><span>V<sub>final</sub></span><input id='v_final' class='param_value' /></label>"
						+ "<label><span>t<sub>delay</sub></span><input id='t_delay' class='param_value' /></label>"
						+ "<label><span>t<sub>rise</sub></span><input id='t_rise' class='param_value' /></label>"
						+ "<label><span>t<sub>fall</sub></span><input id='t_fall' class='param_value' /></label>"
						+ "<label><span>t<sub>high</sub></span><input id='t_high' class='param_value' /></label>"
						+ "<label><span>t<sub>period</sub></span><input id='t_period' class='param_value' /></label></div>";
			$("body").append(tmp_dialog);
		}
		else if($current.attr("type")=="pwl"){
			var tmp_dialog = "<div id='param_dialog' style='display:none;' type='pwl'>"
						+ "<a id='addmore'><i class='icon-plus'/>  </a>"
						+ "<a id='delmore'><i class='icon-minus'/>  </a><br/>"
						+ "<label><span>Name</span><input id='param_name' maxlength='8'/></label>"
						+ "<label><span>T1</span><input id='T1' class='param_value'/></label>"
						+ "<label><span>V1</span><input id='V1' class='param_value'/></label>";//"</div>";
			...
			var tmp_dialog = "<div id='param_dialog' style='display:none;' type='AC'>"
						+ "<label><span>Name</span><input id='param_name' maxlength='8'/></label>"
						+ "<label><span>DC</span><input id='dc_value' class='param_value' /></label>"
						+ "<label><span>AC</span><input id='ac_value' class='param_value' /></label></div>";
			$("body").append(tmp_dialog);
		}
			...
			var tmp_dialog = "<div id='param_dialog' style='display:none;' type='node'>"
						+ "<label><span>Name</span><input id='param_name' maxlength='8'/></label></div>";
			$("body").append(tmp_dialog);
		}
...
function dc_sim(event) {
	...
	$current = $(event.target.parentNode.parentNode);
	var tmp_dialog = "<div id='simulation_dialog' style='display:none;' type='DC Simulation'>"
			+ "<span>Source Name</span><input id='sname' /><br/>"
			+ "<span>V<sub>start</sub></span><input id='sivalue'/><br/>"
			+ "<span>V<sub>stop</sub></span><input id='sevalue'/><br/>"
			+ "<span>V<sub>incr</sub></span><input id='stepcase'/></div>";
...
function ac_sim() {
	...
	$current = $(event.target.parentNode.parentNode);
	var tmp_dialog = "<div id='simulation_dialog' style='display:none;' type='AC Simulation'>"
			+ "<span>increment</span><select id='increment'><option value='lin'>lin</option><option value='dec'>dec</option></select><br>"
			+ "<span>points</span><input id='points'/><br/>"
			+ "<span>fstart</span><input id='fstart'/><br/>"
			+ "<span>fstop</span><input id='fstop'/></div>";
	...
	.dialog({
		...
		buttons:{
			"OK":function(){
			    ...
			    vinput_sim = "* Analysis Definition\n.ac " + $("#increment").val() + " " 
					    + $("#points").val() + " " + $("#fstart").val() + " " + $("#fstop").val() + "\n";
				$(this).dialog("close");
				$(this).remove();
			},
			"Cancel": function () {
...
function tran_sim() {
	...
	$current = $(event.target.parentNode.parentNode);
	var tmp_dialog = "<div id='simulation_dialog' style='display:none;' type='Tran Simulation'>"
			+ "<span>tstep</span><input id='tstep'/><br/>"
			+ "<span>tstop</span><input id='tstop'/><br/>"
			+ "<span>tstart</span><input id='tstart' value='optional'/></div>";
...
function selecteditem(event) {
    ...
    if (event.target.parentNode.id)
    {
        ...
        $current.attr({ "sel": "1" });
        $current.children("path").attr({ "stroke": "red" });
        return;
    }
...
function lrotate(event){
	...
	if($current.attr("type")=="node" || $current.attr("type")=="vnode"){
		...
		switch($current.attr("rota-stat")){
			case "1":
				a=0;b=-1;c=1;d=0;f+=40;
				$current.attr("rota-stat","2");
				break;
			case "2":
				a=-1;b=0;c=0;d=-1;e+=40;
				$current.attr("rota-stat","3");
				break;
			case "3":
				a=0;b=1;c=-1;d=0;f-=40;
			...
			case "5":
				a=0;b=-1;c=-1;d=0;e+=40;
				$current.attr("rota-stat","6");
				break;
			case "6":
				a=-1;b=0;c=0;d=1;f-=40;
		...
		if($current.attr("term")=="2"){
			switch($current.attr("rota-stat")){
				case "1":$current.find(".text").attr("transform","translate("+e+","+f+")");				break;
				case "2":$current.find(".text").attr("transform","translate("+(e-20)+","+(f-60)+")");	break;
				case "3":$current.find(".text").attr("transform","translate("+(e-40)+","+(f-40)+")");	break;
				case "4":$current.find(".text").attr("transform","translate("+(e-60)+","+(f-20)+")");	break;
				case "5":$current.find(".text").attr("transform","translate("+e+","+(f-40)+")");		break;
				case "6":$current.find(".text").attr("transform","translate("+(e-60)+","+(f-60)+")");	break;
				case "7":$current.find(".text").attr("transform","translate("+(e-40)+","+f+")");		break;
				case "8":$current.find(".text").attr("transform","translate("+(e-20)+","+(f-20)+")");	break;
			}
		}
	}
}
...
function rrotate(event){
	...
	if($current.attr("type")=="node" || $current.attr("type")=="vnode"){
		...
		// new a,b,c,d,e,f
		switch($current.attr("rota-stat")){
			case "1": a=1;b=0;c=0;d=-1;f+=80;	
				$current.attr("rota-stat","4");
				$current.find(".text").attr("transform","translate(" + e + "," + (f-35) + ")");
				break;
			case "2": a=1;b=0;c=0;d=1;			
				$current.attr("rota-stat","1");
				$current.find(".text").attr("transform","translate(" + e + "," + f + ")");
				break;
			case "3": a=-1;b=0;c=0;d=1;f-=80;	
				$current.attr("rota-stat","2");
				$current.find(".text").attr("transform","translate(" + (e-nodelen) + "," + f + ")");
				break;
    ...
    $("#workspace").bind({
        ...
        mouseup: function (event) {
            ...
            $(".workspace_component").each(function () {
                ...
                if (parseInt(parts[4]) - tempsize >= temprectx
                    && (parseInt(parts[4]) + parseInt($(this).find("rect").attr("width")) + tempsize) <= (temprectx + parseInt($("#temprect").attr("width")))
                    && parseInt(parts[5]) - tempsize >= temprecty
                    && (parseInt(parts[5]) + parseInt($(this).find("rect").attr("height")) + tempsize) <= (temprecty + parseInt($("#temprect").attr("height"))))
                {
            ...
            $(".workspace_line").each(function () {
                ...
                var parts = transform.match(/(\d+)/g);
                if (parseInt(parts[0]) > parseInt(parts[2])) { var tmp = parts[0]; parts[0] = parts[2]; parts[2] = tmp; }
                if (parseInt(parts[1]) > parseInt(parts[3])) { var tmp = parts[1]; parts[1] = parts[3]; parts[3] = tmp; }
                if (parseInt(parts[0]) >= temprectx
                     && parseInt(parts[0]) <= (temprectx + parseInt($("#temprect").attr("width")))
                     && parseInt(parts[1]) >= temprecty
                     && parseInt(parts[1]) <= (temprecty + parseInt($("#temprect").attr("height"))))
                {
                    if (parseInt(parts[2]) >= temprectx
                         && parseInt(parts[2]) <= (temprectx + parseInt($("#temprect").attr("width")))
                         && parseInt(parts[3]) >= temprecty
                         && parseInt(parts[3]) <= (temprecty + parseInt($("#temprect").attr("height")))) {
                        mulselect = 1;
                    }
                }
...
function check_on_line(x, y) {
    ...
    $(".workspace_line").each(function () {
        ...
        if (parseInt(matches[0]) > parseInt(matches[2])) { var tmp = matches[0]; matches[0] = matches[2]; matches[2] = tmp; }
        if (parseInt(matches[1]) > parseInt(matches[3])) { var tmp = matches[1]; matches[1] = matches[3]; matches[3] = tmp; }
        ...
        if (parseInt(matches[0]) == parseInt(matches[2]) && x == parseInt(matches[0])) {
            for (var i = (parseInt(matches[1]) + 10) ; i <= (parseInt(matches[3]) - 10) ; i += 10) {
                if (y == i) {
                    crossflag = 1;
                    $(this).attr({ "crossid": $(this).attr("crossid") + " " + crossflagid });
                }
...
function check_on_lineinter() {
    ...
    $(".workspace_line").each(function () {
        ...
        $(".workspace_line").each(function () {
            ...
            if ((matches[0] == matches2[0] && matches[1] == matches2[1])
                || (matches[0] == matches2[2] && matches[1] == matches2[3])) {
                tempidname[tempid] = $(this).attr("id");
                tempid++;
            }
            if ((matches[2] == matches2[0] && matches[3] == matches2[1])
                 || (matches[2] == matches2[2] && matches[3] == matches2[3])) {
        ...
        if (tempid > 2) {
            ...
            for(var i=0;i<tempid;i++)
            {
                ...
                $("#" + tempidname[i]).attr({ "crossid": $("#" + tempidname[i]).attr("crossid") + " " + crossflagid });
            }
            var crossdot = makeSVG("circle", {
            	"class":"workspace_dot",
                "r": "2.5",
        ...
        if (tempid2 > 2) {
            ...
            for (var i = 0; i < tempid2; i++) {
                ...
                $("#" + tempidname2[i]).attr({ "crossid": $("#" + tempidname2[i]).attr("crossid") + " " + crossflagid });
            }
            var crossdot = makeSVG("circle", {
            	"class":"workspace_dot",
                "r": "2.5",
		        ...
		        if (matches[0] == matches[2] || matches[1] == matches[3]) {
					...
					if(matches[0] == matches[2]){
						tmp_a = makeSVG("rect",{
							...
							"style"			:"opacity:0",
							"transform"		:"matrix(1,0,0,1," + (parseInt(matches[0])-5) + ","
					...
					//horizontal
						...
						tmp_a = makeSVG("rect",{
							...
							"height"		:"10",
							"style"			:"opacity:0",
							"transform"		:"matrix(1,0,0,1," + Math.min(parseInt(matches[0]),parseInt(matches[2]))
...
function get_netlist(event){
	...
	$(".workspace_component").each(function(index1){
		...
		$(this).find("circle.term").each(function (index2) {
			...
			tmp_array[index2] =
				[parseInt($(this).attr("cx")) * parseInt(parts[0]) + parseInt($(this).attr("cy")) * parseInt(parts[2]) + parseInt(parts[4]),
	...
	/*	alert(component_name[index1]);
		...
		{
			sourcedef+="V"+component_name[index1]+" "+point_to_number[component_term[index1][0]]
				+" "+point_to_number[component_term[index1][1]]+" dc "+component_value[index1]+ '\n';
		}
		else if(component_type[index1]=="dcc")
			sourcedef += "I"+component_name[index1]+" "+point_to_number[component_term[index1][0]]
				+" "+point_to_number[component_term[index1][1]]+" dc "+component_value[index1]+ '\n';
		else if(component_type[index1]=="pulse"){
			sourcedef += "V"+component_name[index1]+" "+point_to_number[component_term[index1][0]]
				+" "+point_to_number[component_term[index1][1]]+" pulse("+component_value[index1]+ ")\n";
		}
		else if(component_type[index1]=="pwl"){
			sourcedef += "V"+component_name[index1]+" "+point_to_number[component_term[index1][0]]
				+" "+point_to_number[component_term[index1][1]]+" pwl("+component_value[index1]+ ")\n";
		}
		else if(component_type[index1]=="ac"){
			sourcedef += component_name[index1]+" "+point_to_number[component_term[index1][0]]
				+" "+point_to_number[component_term[index1][1]]+" "+component_value[index1]+ "\n";
		}
...
function moveLine(event) {
	...
	$current.bind({
		...
		mouseup: function (event) {
			...
			console.log(matches);
			$current.children("path").attr({
				"d":"M " + putGrid(parseInt(matches[0])) + " " + putGrid(parseInt(matches[1])) 
					+ "L" + putGrid(parseInt(matches[2])) + " " + putGrid(parseInt(matches[3]))
			});
			// vertical
			if(matches[0] == matches[2]){
				$current.children("rect").attr({
			...
			setTimeout(function () {
			    ...
			    if (check_on_line(putGrid(parts[0]), putGrid(parts[1]))) {
			        $current.attr({ "crossid": $current.attr("crossid") + " " + crossflagid });
			        crossflagid++;
			    }
